...
export class PaginationService {
    ...
    async checkPagination(payload: Pick<Pagination, 'article_id'>) {
        const find = await this.paginationRepository.findOne({ article_id: new Types.ObjectId(payload.article_id) });
        ...
        if (!find) {
            ...
            await this.eventEmitter.emitAsync('pagination.create', { pagination_id: result._id, article_id: payload.article_id });
        }
    }
}
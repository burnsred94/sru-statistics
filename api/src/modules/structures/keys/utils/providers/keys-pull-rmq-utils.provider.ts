...
export class KeysPullRmqUtilsProvider {
    ...
    private async destructurePvz(array: Array<PvzDocument>, average: Types.ObjectId): Promise<IPvzRMQSendStructure[]> {
        const result = map(array, async (element) => {
            const positionDocument = await this.getLastDocument(element.position) as unknown as PeriodsDocument;
    ...
    private async positionDataDefinition(document: PeriodsDocument): Promise<string> {
        const unspecifiedPositionType = document.position === '1000+' || document.position === 'Нет данных' || document.position === 'Ожидается';

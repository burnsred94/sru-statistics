...
export class UpdateKeywordService {
    ...
    updateCurrentAverage(dataUpdated: Promise<StatisticsUpdateRMQ.Payload>) {
        Promise.all([this.updateCurrentDate, dataUpdated])
            .then(([check, data]) => {
                ...
                this.averageService.update({ id: data.averageId, average: data.position, key_id: data.key_id })
    ...
    async getCheckUpdate() {
        const [checkAverage, checkDate] = await Promise.all([this.checkUpdateCurrentAverage, this.checkUpdateCurrentAverage]);

...
export class KeyBuilder {
  ...
  getFrequency(key: string) {
    this.frequency = this.coreKeysIntegrationService.getFrequency(key).catch(error => {
      this.logger.error({
    ...
    Promise.resolve(this.frequency).then(async frequency => {
      ...
      if (average) {
        this.keysRepository.findOneAndUpdate(
          { _id: keyword._id },
          { $set: { frequency: frequency, ...switchQuery }, $push: { average: average } },
        );
      } else {
        this.keysRepository.findOneAndUpdate(
          { _id: keyword._id },
          { $set: { frequency: frequency, ...switchQuery } },
        );
      }
  ...
  async getDocument(_id?: Types.ObjectId) {
    return _id
      ? this.keysRepository.findOne({ _id }, KeysRefreshPopulate)
      : this.document.then(document => document._id);
  }
}

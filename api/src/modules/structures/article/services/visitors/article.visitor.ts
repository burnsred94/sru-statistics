...
export class ArticleVisitor extends AbstractArticleService {
    ...
    try {
      return Promise.resolve(this.document)
        .then(async document => {
          ...
          const { keys } = await this.articleRepository.findOne(
            { _id: document._id },
            ARTICLE_POPULATE,
          );
  ...
  async activeKeywords(
    keywords: HydratedDocument<Keys>[],
    switchQuery: Active | ActiveSub,
  ): Promise<HydratedDocument<Article>> {
    if (keywords.length === 0) return this.document;

    return Promise.resolve(this.document).then(document => {
      while (keywords.length > 0) {
        const keyword = keywords.shift() as unknown as HydratedDocument<Keys>;
        this.keyBuilder.getFrequency(keyword.key).enabledKeyword(keyword, switchQuery);
      }
      return document;
    });
  }

  async disabledKeywords(
    keywords: HydratedDocument<Keys>[],
    switchQuery: Active | ActiveSub,
  ): Promise<HydratedDocument<Article>> {
    if (keywords.length === 0) return this.document;
    ...
    return Promise.resolve(this.document).then(document => {
      this.articleRepository.findOneAndUpdate(
        { _id: document._id },
        { $inc: { count: -keywords.length } },
      );

      while (keywords.length > 0) {
        const keyword = keywords.shift() as unknown as HydratedDocument<Keys>;
        this.keyBuilder.disabledKeywords(keyword, switchQuery);
      }
      return document;
    });
  }

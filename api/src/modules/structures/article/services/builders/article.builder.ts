...
export class ArticleBuilder extends AbstractArticleService {
    ...
    create(count: number, user: User, article: string) {
        this.document = new Promise((resolve) => {
            const document = this.articleRepository.create({
                article: article,
                userId: user,
        ...
        Promise.all([this.cities, this.document]).then((data) => {
            ...
            from(keywords)
                .pipe(concatMap((key) => {
                    ...
                    builder
                        .create(key, user, article, preparation)
                ...
                .subscribe((data) => {
                    this.articleRepository.findOneAndUpdate({ _id: document._id }, { $push: { keys: data._id } });
                });
        });
    ...
    metricsCreate() {
        Promise.all([this.document, this.cities])
            .then(([document, cities]) => {
    ...
    getProductAndUpdate(article: string) {
        ...
        Promise.all([product, this.document])
            .then((data) => {
                const [product, document] = data;
                console.log(product);
                this.articleRepository.findOneAndUpdate({ _id: document._id },
                    {
    ...
    updateArticle() {
        Promise.resolve(this.document)
            .then((document) => {
                const { keys } = document
                Promise.all(keys.map(async (keyword) => {
                    const builder = this.keyBuilder

                    const document = await builder
                        .getDocument(keyword) as HydratedDocument<Keys>;

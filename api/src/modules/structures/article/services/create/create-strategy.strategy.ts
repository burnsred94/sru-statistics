...
export class CreateArticleStrategy {
    ...
    async findNotActiveAddKeys(article: string, keys, user) {
        ...
        if (find_product.keys.length > 0) {
            ...
            if (matchToActiveKeys.length > 0) await this.actionKey({ keys: matchToActiveKeys, article }, active_product._id._id, user);
    ...
    private async actionKey(data: { keys: string[], article: string }, id: Types.ObjectId, user: User): Promise<void> {
        setImmediate(async () => {
